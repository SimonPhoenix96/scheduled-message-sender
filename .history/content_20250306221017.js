...
async function postDonationMessage(message, selector, shouldSend = true) {
  console.log('Posting message with selector:', selector);
  
  // If using LLM and message is empty, generate a message
  if (currentLLMSettings && currentLLMSettings.useLLM && (!message || message.trim() === '')) {
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.action === 'ping') {
    ...
    if (request.messages && request.messages.length > 0) {
      ...
      // We'll generate a message but not send it
      postDonationMessage("", request.chatSelector, false).then(success => {
        sendResponse({success: success});
      });
  ...
  } else if (request.action === 'submitSingleMessage') {
    console.log('Received submit single message request');
    
    // Submit a single message without setting up intervals
    console.log('Submitting single message with selector:', request.message.selector);
    
    const success = postDonationMessage(request.message.text, request.message.selector, true);
    sendResponse({success: success});
  }
