...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === 'ping') {
      ...
      if (request.messages && request.messages.length > 0) {
        request.messages.forEach((messageObj, index) => {
          ...
          const id = setInterval(() => {
            postDonationMessage(messageObj.text, request.chatSelector);
          }, intervalMs);
      ...
      if (request.messages && request.messages.length > 0) {
        console.log('Message to insert:', request.messages[0].text);
        const success = postDonationMessage(request.messages[0].text, request.chatSelector, false);
