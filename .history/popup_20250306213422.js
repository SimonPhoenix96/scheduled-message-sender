...
function saveCurrentSettings() {
    ...
    chrome.storage.local.get(['activeTabs'], function(data) {
      ...
      chrome.storage.local.set({
        ...
        defaultInterval: intervalInput ? intervalInput.value : 15,
        defaultSelector: chatSelectorInput ? chatSelectorInput.value : '#chat-input-wrapper > div > div.editor-input > p',
        useLLM: useLLMCheckbox.checked,
...
messageTextarea.addEventListener('input', saveCurrentSettings);
intervalInput.addEventListener('change', saveCurrentSettings);
selectorInput.addEventListener('change', saveCurrentSettings);
...
chrome.storage.local.get([
    ...
    if (useLLMCheckbox && useLLMCheckbox.checked && llmSettingsGroup) {
      llmSettingsGroup.classList.remove('hidden');
...
function addAutoSaveListeners() {
    ...
    if (intervalInput) intervalInput.addEventListener('change', saveCurrentSettings);
    if (chatSelectorInput) chatSelectorInput.addEventListener('change', saveCurrentSettings);
    if (useLLMCheckbox) useLLMCheckbox.addEventListener('change', saveCurrentSettings);
    if (llmProviderSelect) llmProviderSelect.addEventListener('change', saveCurrentSettings);
    if (apiKeyInput) apiKeyInput.addEventListener('input', saveCurrentSettings);
    if (contextPromptInput) contextPromptInput.addEventListener('input', saveCurrentSettings);
    
    // Only add MutationObserver if messagesContainer exists
