    ...
    if (request.action === 'start') {
      ...
      intervalId = setInterval(() => {
        if (request.messages && request.messages.length > 0) {
          ...
          const message = request.messages[currentMessageIndex];
          currentMessageIndex = (currentMessageIndex + 1) % request.messages.length;
          
          // Post the message
          postDonationMessage(message, request.chatSelector);
        } else if (currentLLMSettings && currentLLMSettings.useLLM) {
          // If no messages but LLM is enabled, generate a new one
          postDonationMessage("", request.chatSelector);
        }
      }, intervalMs);
